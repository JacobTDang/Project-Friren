...
class RiskAnalyzer:
    ...
    def _generate_overall_assessment(self, position_risk: PositionRisk,
        ...
        if risk_score > 85:
            ...
            assessment = f"⚠️ CAUTION: High risk position, reduced size by {abs(size_change):.0f}%"
            ...
            assessment = f"⚠️ PROCEED WITH CAUTION: Size reduced by {abs(size_change):.0f}% due to risk"
        elif size_change > 10:
    ...
    def get_portfolio_sizing_summary(self, position_sizing_results: List[Dict]) -> Dict[str, Any]:
        ...
        if total_recommended_allocation > 0.95:
            portfolio_recommendations.append("⚠️ Very high portfolio allocation - consider cash reserve")
        ...
        if len([r for r in position_sizing_results if r['risk_category'] in ['HIGH', 'EXTREME']]) > len(position_sizing_results) * 0.3:
            portfolio_recommendations.append("⚠️ High concentration of risky positions")
